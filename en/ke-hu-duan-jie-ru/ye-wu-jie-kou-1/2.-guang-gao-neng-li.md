...
```text
 [IOSLoader openAd:@"adPositioname" adCallback:^(BOOL flag, KTMADType type) {        if (type == KTMADTypePlaque) {            //qlaque        }        if (type == KTMADTypeVideo) {            //Video        }    }];
```
...
```text
Wb.ADManager.Instance.OpenAd("home_mfzs", result =>{        if (result)        {            ShowToast("open success", false);        }        else        {            ShowToast("open fail", true);        }});
```
